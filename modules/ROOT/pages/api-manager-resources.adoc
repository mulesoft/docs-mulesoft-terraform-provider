...
* `spec` - (Required) The Exchange asset specification backing this API instance. See xref:nested-schema-for-spec[Nested Schema for `spec`].
...
* `endpoint` - Endpoint and proxy configuration for the API instance. See xref:nested-schema-for-endpoint[Nested Schema for `endpoint`].
* `deployment` - Deployment target configuration. Auto-populated when `gateway_id` is set. See xref:nested-schema-for-deployment[Nested Schema for `deployment`].
* `routing` - (Block List) Routing rules with weighted upstream backends. Mutually exclusive with `upstream_uri`. See xref:nested-schema-for-routing[Nested Schema for `routing`].
...
* `upstreams` - (Required, Block List) Weighted upstream backends for this route. See xref:nested-schema-for-routing-upstreams[Nested Schema for `routing.upstreams`].
* `label` - A label for this route.
* `rules` - (Block) Match conditions for this route. See xref:nested-schema-for-routing-rules[Nested Schema for `routing.rules`].
...
* `limits` - (Block List) Rate limits for this SLA tier. See xref:nested-schema-for-limits[Nested Schema for `limits`].
...
* `pointcut_data` - Pointcut definition as a JSON string. Restricts the policy to specific resources (methods and/or URIs). When `null`, the policy applies to all resources. Use `jsonencode()` to set this. See xref:pointcut-data[Pointcut Data].
