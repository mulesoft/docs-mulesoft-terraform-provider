...
[WARNING]
====
The top-level `upstream_uri` field is deprecated and will be removed in the next major version. Use the `routing` block in new configurations instead, which supports TLS, labels, multiple upstreams, and weighted routing. See <<migrating-from-upstream-uri>>.
====
