...
* `ingress` - (Block) Ingress configuration for the gateway. See <<Nested Schema for ingress>>.
* `properties` - (Block) Runtime properties for the gateway. See <<Nested Schema for properties>>.
* `logging` - (Block) Logging configuration for the gateway. See <<Nested Schema for logging>>.
* `tracing` - (Block) Distributed tracing configuration for the gateway. See <<Nested Schema for tracing>>.
