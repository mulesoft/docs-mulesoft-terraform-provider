...
resource "anypoint_api_policy_jwt_validation" "example" {
  ...
  configuration = {
    ...
    mandatory_aud_claim             = false
    mandatory_exp_claim             = false
    mandatory_nbf_claim             = false
...
* `environment_id` - ID of the environment where the API instance lives.
* `api_instance_id` - ID of the API instance this policy is applied to.
* `asset_version` - The policy asset version (for example, `1.4.0`). Must match the version deployed in your organization's Exchange.
